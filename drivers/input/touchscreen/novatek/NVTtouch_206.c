...
int32_t CTP_I2C_READ(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len)
{
	...
	if (unlikely(retries == 5)) {
		...
		BBOX_TP_I2C_READ_FAIL	//SW4-HL-TP-BBox-00+_20161221
		ret = -EIO;
	}
...
int32_t CTP_I2C_WRITE(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len)
{
	...
	if (unlikely(retries == 5)) {
		dev_err(&client->dev, "%s: error, ret=%d\n", __func__, ret);
		BBOX_TP_I2C_WRITE_FAIL	//SW4-HL-TP-BBox-00+_20161221
		ret = -EIO;
	}
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ts == NULL) {
		...
		BBOX_TP_PROBE_FAIL	//SW4-HL-TP-BBox-00+_20161221
		return -ENOMEM;
	}
	ts->client = client;
...
static int32_t nvt_ts_suspend(struct device *dev)
{
    ...
    #if NVT_TOUCH_EXT_FIH	//SW4-HL-TP-FixCurrentLeakgeIssue-00+_20161213
	nvt_power_off(ts);
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	//SW4-HL-TP-FixForAmbientDisplayIssue-00*{_20170302
