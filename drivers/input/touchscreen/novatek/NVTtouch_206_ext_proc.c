...
static uint8_t button_num = 0;
	...
	static uint8_t u8FWSubVersion=0;
...
/* Black Box */
#define BBOX_TP_FIRMWARE_VERSION_GET_FAIL	do {printk("BBox;%s: TP firmware version get fail!\n", __func__); printk("BBox::UEC;7::5\n");} while (0);
//SW4-HL-TP-BBox-00+}_20161221
...
info_retry:
	...
	buf[0] = 0x78;
    #if NVT_TOUCH_EXT_FIH
		CTP_I2C_READ(ts->client, I2C_FW_Address, buf, 24);
		fw_ver = buf[1];
		x_num = buf[3];
		y_num = buf[4];
		button_num = buf[11];
		u8FWSubVersion = buf[18];
	#else
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		BBOX_TP_FIRMWARE_VERSION_GET_FAIL	//SW4-HL-TP-BBox-00+_20161221
		x_num = 0;
		y_num = 0;
		button_num = 0;
	#if NVT_TOUCH_EXT_FIH
...
{
	seq_printf(m, "fw_ver=%d, x_num=%d, y_num=%d, button_num=%d, sub_ver=%d\n", fw_ver, x_num, y_num, button_num, u8FWSubVersion);
	return 0;
}
...
void touch_tpfwver_read_nvt(char *fw_ver_fih)
{
	...
	snprintf(fw_ver_fih, 128, "NovaTek-V%d_%d\n", fw_ver, u8FWSubVersion);
}
//SW4-HL-TP-FTM-00+}_20161110
#endif//NVT_TOUCH_EXT_FIH