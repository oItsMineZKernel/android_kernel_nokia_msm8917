...
static void fts_psensor_enable(struct fts_ts_data *data, int enable)
{
	...
	ret = fts_read_reg(data->client, FTS_REG_PSENSOR_ENABLE, &state);
    	if (ret < 0){
            	printk("[focal] %s fts_read_reg fail\n", __func__);
		printk("BBox::UEC;7::1\n");
    	}
	if (enable)
...
static irqreturn_t fts_ts_interrupt(int irq, void *dev_id)
{
	...
	// FIHTDC@Ivan - To fix sometimes interrupt can't work issue - S
	enable_irq(fts_wq_data->client->irq);
	// FIHTDC@Ivan - To fix sometimes interrupt can't work issue - E
...
static int fts_read_Touchdata(struct fts_ts_data *data)
{
	...
	if(data->suspended)
	{
		ret = fts_read_reg(data->client, 0xd0, &state);
            	if (ret < 0){
            	    printk("[focal] %s fts_read_reg fail \n", __func__);
		    printk("BBox::UEC;7::1\n");
		}
...
static int j = 1;	//SW4-HL-TouchPanel-FixFT5446SometimeNotWorkIssue-00+_20151208
/*******************************************************************************
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	for (i = 0; i < FTS_MAX_POINTS; i++) {
		...
		//SW4-HL-TouchPanel-FixFT5446SometimeNotWorkIssue-00*{_20151208
		#if 0
		if(0 == event->pressure[i])
			event->pressure[i] = 0x3f;
		#else
		//Fix reported pressure value from 1 to 50 when user touches the panel every time
		if(0 == event->pressure[i])
		{
			if (j >= 51)
			{
...
static int fts_power_init(struct fts_ts_data *data, bool on)
{
	...
	if (!on)
	{
		...
		goto power_deinit;	//SW4-HL-TouchPanel-FixOS&FTMCan'tPowerOff-00+_20151222
	}
	...
	/* //Fihtdc@AlanHZ, Change to Fixed regulator
	...
	*/
	...
	if (IS_ERR(data->vcc_i2c)) {
		...
		//goto reg_vdd_set_vtg;	//SW4-HL-TouchPanel-FixOS&FTMCan'tPowerOff-00-_20151222
	}
...
//SW4-HL-TouchPanel-FixOS&FTMCan'tPowerOff-00*{_20151222
power_deinit:
	dev_err(&data->client->dev, "power_deinit\n");
...
static int fts_ts_stop(struct device *dev)
{
	...
	*/
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		...
		err = fts_i2c_write(data->client, txbuf, sizeof(txbuf));
		    if (err < 0) {
...
int fts_ts_suspend(struct device *dev)
{
	...
	pr_err("\n\n[HL]%s <-- end, return fts_ts_stop(dev)\n\n", __func__);
...
void fih_fts_tp_lcm_suspend(void)
{
	if(!tp_probe_success)
	{
		pr_err("\n[HL]%s , tp_probe_success = %d, focal tp probe failed, ignore suspend\n", __func__, tp_probe_success);
		return;
	}
...
//Mark this if-else to fix focaltech can't sleep in FTM mode
//#if (!defined(CONFIG_FB) && !defined(CONFIG_HAS_EARLYSUSPEND))	//SW4-HL-TouchPanel-FixOS&FTMCan'tPowerOff-00-_20151222
	//.suspend = fts_ts_suspend,
	//.resume = fts_ts_resume,
	.suspend = NULL,
	.resume = NULL,
//#endif															//SW4-HL-TouchPanel-FixOS&FTMCan'tPowerOff-00-_20151222
};
#else
/*******************************************************************************
*  Name: fts_ts_suspend
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//SW4-HL-TouchPanel-ExitProbeFunctionIfTouchPanelDoesNotExist-00*{_20151126
	...
	if (err < 0) {
		...
		pr_err("%s: ID READ FAIL, EXIT PROBE!\n", __func__);
		goto free_gpio;	//SW4-HL-TouchPanel-FixOS&FTMCan'tPowerOff-00*_20151222
	}
...
/*
    ...
    */
	...
	dev_info(&client->dev, "FT5446 tp probe OK.\n");
