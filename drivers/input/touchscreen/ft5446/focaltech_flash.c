...
int hidi2c_to_stdi2c(struct i2c_client * client)
{
	...
	bRet =fts_i2c_write(client, auc_i2c_write_buf, 3);
    		if (bRet < 0) {
		    printk("[focal] %s fts_i2c_write fail \n", __func__);
		    printk("BBox::UEC;7::2\n");
    		}
	msleep(10);
	auc_i2c_write_buf[0] = auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = 0;
	bRet = fts_i2c_read(client, auc_i2c_write_buf, 0, auc_i2c_write_buf, 3);
        		if (bRet < 0) {
        		    printk("[focal] %s fts_i2c_read fail \n", __func__);
        		    printk("BBox::UEC;7::1\n");
        		}
	if(0xeb==auc_i2c_write_buf[0] && 0xaa==auc_i2c_write_buf[1] && 0x08==auc_i2c_write_buf[2])
	{
		pr_debug("hidi2c_to_stdi2c successful.\n");
		bRet = 1;
	}
	else
	{
...
int fts_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client, u8 *ucPVendorID)
{
	...
	*ucPVendorID = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		...
		if (i_ret < 0) {
                	printk("[focal] %s fts_write_reg fail 1\n", __func__);
                	printk("BBox::UEC;7::2\n");
		}
		msleep(fts_updateinfo_curr.delay_aa);
		i_ret = fts_write_reg(client, 0xfc, FTS_UPGRADE_55);
		if (i_ret < 0) {
                	printk("[focal] %s fts_write_reg fail 2\n", __func__);
                	printk("BBox::UEC;7::2\n");
		}
		msleep(200);
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1 && reg_val[1] == fts_updateinfo_curr.upgrade_id_2) {
			pr_debug("[FTS] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]);
			break;
		}
		else
		{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		i_ret = fts_i2c_write(client, auc_i2c_write_buf, 4);
        		if (i_ret < 0) {
        		    printk("[focal] %s fts_i2c_write fail 3\n", __func__);
        		    printk("BBox::UEC;7::2\n");
        		}
		msleep(5);
		reg_val[0] = reg_val[1] = 0x00;
		i_ret = fts_i2c_read(client, auc_i2c_write_buf, 0, reg_val, 2);
        		if (i_ret < 0) {
        		    printk("[focal] %s fts_i2c_read fail 2\n", __func__);
        		    printk("BBox::UEC;7::1\n");
        		}
		if (0 != reg_val[0])
		{
	...
	pr_debug("Step 5: reset the new FW\n");
	auc_i2c_write_buf[0] = 0x07;
	i_ret = fts_i2c_write(client, auc_i2c_write_buf, 1);
            		if (i_ret < 0) {
        		    printk("[focal] %s fts_i2c_write fail 4 \n", __func__);
        		    printk("BBox::UEC;7::2\n");
        		}
	msleep(200);
	i_ret = hidi2c_to_stdi2c(client);
...
int fts_ctpm_fw_upgrade_ReadProjectCode(struct i2c_client *client, char *pProjectCode)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		...
		i_ret = fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
                	if (i_ret < 0) {
                	    printk("[focal] %s fts_i2c_read fail 1 \n", __func__);
                	    printk("BBox::UEC;7::1\n");
        		}
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1 && reg_val[1] == fts_updateinfo_curr.upgrade_id_2)
		{
	...
	for (j = 0; j < 33; j++)
	{
		...
		i_ret = fts_i2c_read(client, auc_i2c_write_buf, 4, pProjectCode+j, 1);
		if (*(pProjectCode+j) == '\0')
			break;
	}
                	if (i_ret < 0) {
...
void fts_get_upgrade_array(void)
{
	...
	//SW4-HL-TouchPanel-FixFirmwareCan'tUpgradedAgainIfFirmwareUpgradeFail-00*{_20151209
	//Fix to set chip id as FT5x46 if chip id read fail
	if(i >= sizeof(fts_updateinfo)/sizeof(struct fts_Upgrade_Info))
	{
		memcpy(&fts_updateinfo_curr, &fts_updateinfo[11], sizeof(struct fts_Upgrade_Info));	//fts_updateinfo[11] is for FT5446
	}
	//SW4-HL-TouchPanel-FixFirmwareCan'tUpgradedAgainIfFirmwareUpgradeFail-00*}_20151209
}
...
int fts_ctpm_auto_clb(struct i2c_client *client)
{
	...
	i_ret = fts_write_reg(client, 2, 0x4);
		if (i_ret < 0) {
		    printk("[focal] %s fts_write_reg 2\n", __func__);
		    printk("BBox::UEC;7::2\n");
		}
	msleep(300);
	if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) ||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14)) //5x36,5x36i
	{
		for(i=0;i<100;i++)
		{
				i_ret = fts_read_reg(client, 0x02, &uc_temp);
                		if (i_ret < 0) {
                		    printk("[focal] %s fts_read_reg fail 1\n", __func__);
                		    printk("BBox::UEC;7::1\n");
                		}
			if (0x02 == uc_temp ||
				0xFF == uc_temp)
	...
	else
	{
		for(i=0;i<100;i++)
		{
				i_ret = fts_read_reg(client, 0, &uc_temp);
                		if (i_ret < 0) {
                		    printk("[focal] %s fts_read_reg fail 2\n", __func__);
                		    printk("BBox::UEC;7::1\n");
                		}
	...
	i_ret = fts_write_reg(client, 0, 0x40);
    		if (i_ret < 0) {
                	printk("[focal] %s fts_write_reg fail 3\n", __func__);
                	printk("BBox::UEC;7::2\n");
		}
	msleep(200);
	i_ret = fts_write_reg(client, 2, 0x5);
    		if (i_ret < 0) {
                	printk("[focal] %s fts_write_reg fail 4\n", __func__);
                	printk("BBox::UEC;7::2\n");
		}
	msleep(300);
	i_ret = fts_write_reg(client, 0, FTS_WORKMODE_VALUE);
    		if (i_ret < 0) {
                	printk("[focal] %s fts_write_reg fail 5\n", __func__);
                	printk("BBox::UEC;7::2\n");
		}
	msleep(300);
	return 0;
...
int fts_6x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	if(pbt_buf[0] != 0x02)
	{
		pr_err("[FTS] FW first byte is not 0x02. so it is invalid \n");
		return -1;
	}
	...
	if(dw_lenth > 0x11f)
	{
		...
		{
			pr_err("[FTS] Fw length is invalid \n");
			return -1;
		}
	}
	else
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		...
		{
			pr_debug("[FTS] Step 3: GET CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		}
		else
		{
			pr_debug("[FTS] Step 3: GET CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
		}
	}
	if (i >= FTS_UPGRADE_LOOP)
	...
	for (j = 0; j < packet_number; j++)
	{
		...
		for(i = 0;i < 30;i++)
		{
			...
			{
				pr_err("[FTS] write a block data finished \n");
				break;
			}
			msleep(1);
		}
	...
	if (reg_val[0] != bt_ecc)
	{
		pr_err("[FTS]--ecc error! FW=%02x bt_ecc=%02x\n",
					reg_val[0],
					bt_ecc);
		return -EIO;
	}
...
int fts_6x06_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2)
		{
			pr_debug("[FTS] Step 3: CTPM ID OK ,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
			break;
		}
		else
		{
			pr_debug("[FTS] Step 3: CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
		}
	}
	if (i > FTS_UPGRADE_LOOP)
		return -EIO;
...
int fts_5x26_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	pr_debug("Step 8: Disable Write Flash\n");
    	auc_i2c_write_buf[0] = 0x04;
    	fts_i2c_write(client, auc_i2c_write_buf, 1);
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		...
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] = 0x00;
              ...
              {
                      pr_debug("[FTS] Step 3: CTPM ID FAILD,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
                      continue;
              }
...
int fts_5822_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < 100; i++)
	{
		...
		reg_val[0] = reg_val[1] = 0x00;
		fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
		pr_debug("[FTS]--reg_val[0]=%02x reg_val[0]=%02x\n", reg_val[0], reg_val[1]);
...
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
		...
		for (i = 0; i < FTS_PACKET_LENGTH; i++)
	{
		...
		if(i_ret < 0)
		{
        	    printk("[focal] %s fts_i2c_read fail 3\n", __func__);
		    printk("BBox::UEC;7::1\n");
		}
			if ((j + 0x1000) == (((reg_val[0]) << 8) | reg_val[1]))
			{
		...
		for (i = 0; i < temp; i++)
	{
		...
		if(i_ret < 0)
		{
        	    printk("[focal] %s fts_write_reg fail 6\n", __func__);
		    printk("BBox::UEC;7::2\n");
		}
		for(i = 0;i < 30;i++)
		{
			...
			i_ret = fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
            		...
            		if(i_ret < 0)
            		{
            		    printk("[focal] %s fts_i2c_read fail 4 \n", __func__);
            		    printk("BBox::UEC;7::1\n");
            		}
			pr_debug("[FTS][%s] reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
			if ((j + 0x1000) == (((reg_val[0]) << 8) | reg_val[1]))
	...
	i_ret = fts_i2c_write(client, auc_i2c_write_buf, 1);
		if(i_ret < 0)
		{
        	    printk("[focal] %s fts_write_reg fail 7\n", __func__);
		    printk("BBox::UEC;7::2\n");
		}
	msleep(300);
	...
	i_ret = fts_i2c_write(client, auc_i2c_write_buf, 6);
		if(i_ret < 0)
		{
        	    printk("[focal] %s fts_write_reg fail 8\n", __func__);
		    printk("BBox::UEC;7::2\n");
		}
	msleep(dw_lenth/256);
	...
	for(i = 0;i < 100;i++)
	{
		...
		if(i_ret < 0)
		{
        	    printk("[focal] %s fts_i2c_read fail 5\n", __func__);
		    printk("BBox::UEC;7::1\n");
		}
		pr_debug("[FTS]--reg_val[0]=%02x reg_val[0]=%02x\n", reg_val[0], reg_val[1]);
		if (0xF0==reg_val[0] && 0x55==reg_val[1])
	...
	i_ret = fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 1);
  		if(i_ret < 0)
		{
        	    printk("[focal] %s fts_i2c_read fail 6\n", __func__);
		    printk("BBox::UEC;7::1\n");
		}
	if (reg_val[0] != bt_ecc)
	{
	...
	fts_i2c_write(client, auc_i2c_write_buf, 1);
		if(i_ret < 0)
		{
        	    printk("[focal] %s fts_write_reg fail 9\n", __func__);
		    printk("BBox::UEC;7::2\n");
		}
	msleep(200);
	i_ret = hidi2c_to_stdi2c(client);
...
int  fts_8606_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	pr_err("8606 dw_lenth= %d",dw_lenth);
	...
	pr_err("checksum %X %X \n",reg_val[0],bt_ecc);
	pr_err("Read flash and compare\n");

	msleep(50);

	/*********Step 6: start app***********************/
	pr_debug("Step 6: start app\n");
...
int  fts_8606_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	{
		temp = dw_lenth;
		pr_debug("Step 6_1: read out checksum\n");
	}
	auc_i2c_write_buf[4] = (u8)(temp >> 8);
	auc_i2c_write_buf[5] = (u8)(temp);
	i_ret = fts_i2c_write(client, auc_i2c_write_buf, 6);
...
int  fts_8716_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	pr_err("8716 dw_lenth= %d",dw_lenth);
	if(dw_lenth > 0x10000 || dw_lenth ==0)
	{
		return -EIO;
	}
	...
	for (i = 0; i < 20; i++)
	{
		...
		if ((reg_val[0] == 0x87
			&& reg_val[1] == 0x16))
		{
			pr_debug("[FTS] Step 3: READ CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		}
		else
		{
			pr_debug("[FTS] Step 3: READ CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
...
int  fts_8716_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if ((reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			...
			pr_debug("[FTS] Step 3: READ FAIL CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			continue;
		}
	}
...
static int fts_GetFirmwareSize(char *firmware_name)
{
	...
	if (NULL == pfile)
	{
		pfile = filp_open(filepath, O_RDONLY, (S_IRUGO | S_IWUSR) );
	}
	if (IS_ERR(pfile))
	{
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	//Compare firmware file version(/system/etc/firmware/TP_Firmware) and IC's firmware version
	//Get firmware file's firmware version
	fw_ver_of_firmware_file = pbt_buf[fwsize-2];

	//Get IC's firmware version
	if (fts_read_reg(client, FTS_REG_FW_VER, &fw_ver_of_ic) < 0)
	{
		return -EIO;
	}
	if (fw_ver_of_ic == 255)
...
int fts_ctpm_fw_upgrade_with_app_file_fih(struct i2c_client *client, char *firmware_name)
{
	u8 *pbt_buf = NULL;
	int retry_cnt, i_ret=0;
	int fwsize = fts_GetFirmwareSize(firmware_name);
	...
	if (!gForceUpgrade)
	{
			pr_err("START to upgrade tp firmware!\n");
...
//SW4-HL-TouchPanel-GetFirmwareFileVersion-00+{_20151118
unsigned char fts_ctpm_get_app_file_ver(struct i2c_client *client, char * firmware_name)
{
	u8 *pbt_buf = NULL;
	int fwsize = fts_GetFirmwareSize(firmware_name);
	...
	if (pbt_buf)
	{
		...
		fwver = pbt_buf[fwsize-2];
		kfree(pbt_buf);
	}
	else
	{
		pr_info("\n\n[HL]%s: pbt_buf is FALSE\n\n", __func__);
	}
...
int fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	{
		i_ret = fts_write_reg(client, 0xbc, 0xaa);
                    		if (i_ret < 0) {
                	printk("[focal] %s \n", __func__);
		printk("BBox::UEC;7::2\n");
		}
	...
	{
		i_ret = fts_write_reg(client, 0xfc, 0xaa);
                    		if (i_ret < 0) {
                	printk("[focal] %s \n", __func__);
		printk("BBox::UEC;7::2\n");
		}
	...
	if(fts_updateinfo_curr.CHIP_ID==0x06 || fts_updateinfo_curr.CHIP_ID==0x36)
	{
		i_ret = fts_write_reg(client, 0xbc, 0x55);
                    		if (i_ret < 0) {
                	printk("[focal] %s \n", __func__);
		printk("BBox::UEC;7::2\n");
		}
	...
	{
		i_ret = fts_write_reg(client, 0xfc, 0x55);
                    		if (i_ret < 0) {
                	printk("[focal] %s \n", __func__);
		printk("BBox::UEC;7::2\n");
		}
	...
	i_ret = fts_i2c_read(client, buf, 4, buf, FTS_SETTING_BUF_LEN);
    		if(i_ret < 0)
		{
        	printk("[focal] %s \n", __func__);
		printk("BBox::UEC;7::1\n");
	...
	i_ret = fts_i2c_write(client, packet_buf, FTS_SETTING_BUF_LEN + 6);
            		if(i_ret < 0)
		{
        	printk("[focal] %s \n", __func__);
		printk("BBox::UEC;7::2\n");
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		...
		{
			if(fts_updateinfo_curr.AUTO_CLB==AUTO_CLB_NEED)
			{
