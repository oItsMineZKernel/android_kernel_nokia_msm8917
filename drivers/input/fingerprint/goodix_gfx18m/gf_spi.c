    ...
    for(i = 0; i< ARRAY_SIZE(key_map); i++) {
        if(key_map[i].val == gf_key.key){
            if(gf_dev->key_customer_define == 1){
              input_report_key(gf_dev->input, key_map[i].key_customer1, gf_key.value); //FihtdcCode@AlanHZ, Add for key define by customer
              pr_debug("key_map[i].key_customer1=[%d].\n",key_map[i].key_customer1);
            }else if(gf_dev->key_customer_define == 2){
              input_report_key(gf_dev->input, key_map[i].key_customer2, gf_key.value); //FihtdcCode@AlanHZ, Add for key define by customer
...
static int __init gf_init(void)
{
	int status;
	FUNC_ENTRY();
	if(strstr(saved_command_line, "androidboot.fp=goodix") == NULL || (strstr(saved_command_line,"androidboot.mode=charger") != NULL))
	{
