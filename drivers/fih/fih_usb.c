...
static int check_Allport_num=0;
...
#ifdef EXTER_PROJECT_PID_VID
		case 0x7A15: //For MO7,ptp,mass_storage
		case 0x7A16: //For MO7,ptp,mass_storage,adb
#endif
			sw_pid.state = -1;
...
static void usb_all_port_setting(struct work_struct *work)
{
	...
	pr_info("%s: into usb_all_port_setting and check_Allport_num %d\n", __func__,check_Allport_num);
	if ((android_usb_product_id() != 0xC000) && (check_Allport_num<MAX_CHECK_NUM))
	{
	       if(selinux_enforcing != 0) {
	       ...
	       fih_switch_all_port();
	       check_Allport_num = check_Allport_num+1;
	       queue_delayed_work(info->usb_workqueue, &info->usb_work, msecs_to_jiffies(10000));
	}
}
