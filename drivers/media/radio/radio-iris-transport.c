...
#include <linux/delay.h> /*fihtdccode@CPLai 20150602 add for fm radio retry*/
...
static int radio_hci_smd_register_dev(struct radio_data *hsmd)
{
	struct radio_hci_dev *hdev;
	int rc;
	int rty_count=0; /*fihtdccode@CPLai 20150602 add for fm radio retry*/
	FMDBG("hsmd: %pK", hsmd);
...
/*fihtdcode@20140213 AlanYAChung added for FM self calibration test begin */
	do{
		rc = smd_named_open_on_edge("APPS_FM", SMD_APPS_WCNSS,
		&hsmd->fm_channel, hdev, radio_hci_smd_notify_cmd);
...
/*fihtdcode@20140213 AlanYAChung added for FM self calibration test end */
	if (rc < 0) {
