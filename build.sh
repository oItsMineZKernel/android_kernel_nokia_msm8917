...
{
    clean
    noquotes "You Need to Run Script with \"sudo .\/build.sh\" to Install all Dependencies"
    separator
...
detect_env ()
{
    ...
    if ! test -f "build/AIK/ramdisk/init"; then
        ...
        curl -LOSs "${REPO_URL}ramdisk/inits" && mv *its build/AIK/ramdisk/init
        check "Ramdisk Binary"
    fi

    if ! test -f "build/module-binary"; then
        quotes "Getting Module Binary"
        curl -LOSs "${REPO_URL}module-binary" && mv mo* build
        check "Module Binary"
    fi
